...
########################################################################################################################################
def CollectDownloadLinkofS2L1Cproducts_GC(ROI, SensingPeriod, S2CatalogueFolder):
    """
    ...
    text_file = open('S2L1C_Products/S2L1CProducts_URLs.txt', "wt")
    text_file.write('\n'.join(urls) + "\n")
    text_file.close()
...
########################################################################################################################################
def CollectDownloadLinkofS2L1Cproducts_COAH(COAHuser, COAHpass, ROI, SensingPeriod):
    """
...
def CleanAndOrganizeACOLITE(AcoliteFolder,S2L1CproductsFolder,SAFEFileName):
    ...
    if len(ListOfACOLITEfilePaths) == 0:
        ...
        if os.path.exists(os.path.join(AcoliteFolder, ACOLITEProductFolderName)):
            ...
            # Stacks tif bands into a single tif
            SortingPattern = ["B01","B02","B03","B04","B05","B06","B07","B08","B8A","B11","B12"] # Prevents confusion between B08 and B8A during sort.
            ACOLITE_ProductsName = os.path.basename(ACOLITEProductFolder)
            ListOfBandPaths = [os.path.join(ACOLITEProductFolder, Band+".tif") for Band in SortingPattern]
            
            VirtualStack = gdal.BuildVRT('', ListOfBandPaths, separate=True)
            gdal.Translate(os.path.join(ACOLITEProductFolder, ACOLITE_ProductsName+'_StackedBands.tif'), VirtualStack, format='GTiff')
            VirtualStack = None
    return LogList
