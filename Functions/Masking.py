...
from Functions.SpectralIndices import CalculateNormalizedIndexTif
from Functions.Auxiliar import GenerateTifPaths

#######################################################################################################################################
def Create_Mask_fromWCMaps(WorldCoverMapsFolder, DstEPSG, Bounds, SpatialRes, MaskFolder, WCnonExistTile, BufferSize=0):
    """
    ...
    Input: WorldCoverMapsFolder - Path to the folder where the WorldCover maps are saved.
           ...
           MaskFolder - Folder where the final mask will be saved.
    ...
    if WCnonExistTile == True:
        ...
        MaskPath = os.path.join(MaskFolder, os.path.basename(MaskFolder) + "_WATER_Mask.tif")
...
#################################################################################################################################
def Create_Mask_fromBand8(ACOLITEoutputsFolder,ACOLITESAFEname, MaskingProductFolder, Band8threshold,Band8Dilation_Size):
    """
    This function creates a mask based on the band 8 from the ACOLITE corrected band 8.
    The mask is created by thresholding the band 8 values values by a threshold (i.e. 0.03) to eliminate water-only pixels, and
    subsequently applying a morphological dilation on the binary mask, to create a safety buffer and be
    sure that the eventual floating material is not exlcuded.
    Input: ProductToMask - Path to the ACOLITE product folder where the Band (8) is saved. String.
           MaskingProductFolder - Folder where the masks will be saved. String.
    ...
    """
    #Get band 
    ProductToMask = os.path.join(ACOLITEoutputsFolder,ACOLITESAFEname, 'B08.tif')
    
    # Apply a thresholding on the band and get a binary mask
    Open_Band8 = gdal.Open(ProductToMask)
    Band8_Data = Open_Band8.GetRasterBand(1).ReadAsArray()
    Band8_Thresholding = Band8_Data > Band8threshold
    
    # Save thresholded mask
    MaskB8_Thr_PathAndTifName = os.path.join(MaskingProductFolder, ACOLITESAFEname + "_Band8_Thr.tif")
    Driver = gdal.GetDriverByName("GTiff")        
    NIraster = Driver.Create(MaskB8_Thr_PathAndTifName, Open_Band8.RasterXSize, Open_Band8.RasterYSize, 1, gdal.GDT_Byte)
    NIraster.SetProjection(Open_Band8.GetProjectionRef())
    NIraster.SetGeoTransform(Open_Band8.GetGeoTransform())
    NIrasterBand = NIraster.GetRasterBand(1)
    ...
    MaskB8_Dil_PathAndTifName = os.path.join(MaskingProductFolder, ACOLITESAFEname + "_Band8_Thr_Dil_Mask.tif")
...
########################################################################################################################################  
def CloudMasking_S2CloudLess_ROI_10m(ACOLITEoutputsFolder,ACOLITESAFEname, MaskingProductFolder,S2CL_Threshold,S2CL_Average,S2CL_Dilation):
    """
    This function create cloud masks on Sentinel-2 Level-1C products based on s2_cloudless algorithm.
    ...
    """
    # Get shape and reprojection info from reference band
    ReferenceImage = os.path.join(ACOLITEoutputsFolder,ACOLITESAFEname, 'Top_Atmosphere_Bands', 'rhot_B02.tif')
    with rasterio.open(ReferenceImage) as scl:
        ref = scl.read()
        aff = scl.transform
        crs = scl.crs
    ...
    for band_rhot in SortingPattern:
        ListofImages_rhot.append(os.path.join(ACOLITEoutputsFolder,ACOLITESAFEname, 'Top_Atmosphere_Bands', band_rhot  + '.tif'))
    ...
    Mask = Cloud_Detector.get_cloud_masks(Bands).astype(rasterio.uint8)
    # Write output cloud mask 
    tif_out_image = os.path.join(MaskingProductFolder, os.path.basename(MaskingProductFolder) + '_CLOUD_Mask_10m.tif')
    with rasterio.open(tif_out_image,"w",  driver='GTiff', compress="lzw", height=Mask.shape[1], width=Mask.shape[2], count=1, dtype=rasterio.uint8, transform=aff, crs=crs) as dest:
        dest.write(Mask)
    # Write output cloud probability (Comment if needed)
    #with rasterio.open(os.path.join(MaskingProductFolder, os.path.basename(MaskingProductFolder) + '_CLOUD_Prob_10m.tif'), "w",  driver='GTiff',compress="lzw",height=Cloud_Probs.shape[1],width=Cloud_Probs.shape[2],count=1,dtype=Cloud_Probs.dtype,nodata=255, transform=aff, crs=crs) as dest:
    #    dest.write(Cloud_Probs)
...
#######################################################################################################################################
def CreateFinalMask(MaskingProductFolder, NDWI_or_Band8_andS2cloudlessUIn = ['BAND8', False]):
    """
    ...
    Input: MaskingProductFolder - Folder where the masks are saved and will be saved. String.
    ...
    # Final Mask Path
    Name = os.path.basename(MaskingProductFolder)
    ...
    if NDWI_or_Band8_andS2cloudlessUIn[0] == 'BAND8' and NDWI_or_Band8_andS2cloudlessUIn[1] == True:
